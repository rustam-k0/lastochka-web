...
function PanelContent({ type, id }: { type: string; id?: string }) {
  ...
  useEffect(() => {
    if (type === "time")
      checkout.slots(state.fulfillment.mode).then((x) => {
        setSlots(x);
        setDay(x[0]?.date || "");
      });
    ...
    return (
      <div className={s.stack}>
        <p className={s.explain}>
          ...
          референса, очистит адреса, избранное и историю заказов. Общая сумма —
          1 037,40 ₽.
        </p>
        <button
          className={p.primary}
    ...
    return (
      <div className={s.stack}>
        <div className={s.document}>
          ...
          <p>
            Это демонстрация интерфейса. Данные сохраняются только в вашем
            браузере. Реальные покупки, доставка и оплата не выполняются.
          </p>
          <h3>Публичная оферта</h3>
          <p>
            Этот раздел — демонстрационный образец, а не юридическая оферта
            магазина. Оформление создаёт локальную запись и не заключает договор
            купли-продажи.
          </p>
          <h3>Ваши данные</h3>
          <p>
            Корзина, адреса и тестовые заказы хранятся на этом устройстве. Для
            удаления можно очистить данные сайта в браузере.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className={s.faq}>
        {[
          [
            "Это настоящий заказ?",
            "Нет, заказы создаются только в этом браузере и не передаются магазину.",
          ],
          [
